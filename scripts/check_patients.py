...
def check_permissions():
    ...
    with app.app_context():
        ...
        for patient in patients:
            ...
            print(
                f"Patient {patient.id}: {patient.first_name} {patient.last_name} - Accessible to nurse1: {has_access}"
            )
            if has_access:
        ...
        for patient in accessible_patients:
            ...
            for assessment in assessments:
                print(
                    f"    Assessment {assessment.id}: {assessment.assessment_date} - Protocol ID: {assessment.protocol_id}"
                )
