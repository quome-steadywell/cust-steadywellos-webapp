#!/usr/bin/env python3
"""
Script to update database protocols with content from the knowledge base.
This extracts real protocol content from the ingested PDF documents and 
updates the hardcoded database protocols.
"""
...
def extract_protocol_content_from_knowledge():
    """Extract protocol content from knowledge base documents."""
    ...
    # Search for different protocol types in the knowledge base
    protocol_searches = {
        'CANCER': {
            'query': 'cancer palliative care pain management oncology',
            'title_keywords': ['cancer', 'oncology', 'palliative']
        },
        'HEART_FAILURE': {
            'query': 'heart failure cardiology dyspnea edema',
            'title_keywords': ['heart', 'failure', 'cardiac']
        },
        'COPD': {
            'query': 'COPD chronic obstructive pulmonary disease respiratory',
            'title_keywords': ['copd', 'pulmonary', 'respiratory']
        },
        'FIT': {
            'query': 'telephone triage protocols nursing assessment',
            'title_keywords': ['telephone', 'triage', 'protocol', 'nursing']
        }
    }
    
    extracted_content = {}
    
    for protocol_type, search_config in protocol_searches.items():
        logger.info(f"Searching knowledge base for {protocol_type} protocol content...")
        
        # Search the knowledge base
        results = knowledge_service.search(search_config['query'], k=10)
        
        if not results:
            logger.warning(f"No knowledge base results found for {protocol_type}")
            continue
            
        # Find the most relevant document, prioritizing by title keywords
        best_result = None
        best_score = 0
        ...
        for result in results:
            ...
            relevance_score = result['score'] if result['score'] else 0
            
            # Boost score if title contains our keywords
            for keyword in search_config['title_keywords']:
        ...
        if best_result:
            ...
            # Create structured protocol content
            extracted_content[protocol_type] = {
            ...
            logger.info(f"Found content for {protocol_type} from '{metadata.get('title', 'Unknown')}'")
        else:
            logger.warning(f"No suitable content found for {protocol_type}")
    
    return extracted_content


def generate_protocol_structure(content_text, protocol_type):
    """Generate structured protocol data from content text with 15 comprehensive questions."""
    
    questions = []
    interventions = []
    decision_tree = []
    
    # Create comprehensive 15-question assessments based on protocol type
    if protocol_type == 'CANCER':
        questions = [
    ...
    elif protocol_type == 'HEART_FAILURE':
        questions = [
...
def update_protocols_with_knowledge_content():
    """Update database protocols with content from knowledge base."""
    
    logger.info("Starting protocol update from knowledge base...")
    
    # Extract content from knowledge base
    extracted_content = extract_protocol_content_from_knowledge()
    
    if not extracted_content:
        logger.error("No content extracted from knowledge base")
        return False
    
    # Update each protocol in the database
    updated_count = 0
    
    for protocol_type_str, content_data in extracted_content.items():
        try:
            # Get the protocol from database
            protocol_type_enum = ProtocolType(protocol_type_str.lower())
            protocol = Protocol.query.filter_by(protocol_type=protocol_type_enum).first()
            
            if not protocol:
                logger.warning(f"No database protocol found for type {protocol_type_str}")
                continue
            
            # Generate structured protocol data
            questions, interventions, decision_tree = generate_protocol_structure(
                content_data['content'], 
                protocol_type_str
            )
            
            # Update protocol with new content
            original_name = protocol.name
            
            # Use standardized protocol names (preserve normalized names)
            from src.utils.protocol_names import ensure_protocol_name_consistency
            if ensure_protocol_name_consistency(protocol):
                logger.info(f"Updated protocol name from '{original_name}' to '{protocol.name}'")
            protocol.description = f"Protocol derived from: {content_data['source_document']}\n\n{content_data['content'][:500]}..."
            
            # Update with comprehensive protocol structure
            # First clear existing data, then set new data
            protocol.questions = []
            protocol.interventions = []
            protocol.decision_tree = []
            db.session.flush()  # Force update to DB
            
            # Now set the new comprehensive data
            protocol.questions = questions
            protocol.interventions = interventions
            protocol.decision_tree = decision_tree
            
            # Increment version
            protocol.version = f"{float(protocol.version) + 0.1:.1f}"
            
            logger.info(f"Updated protocol '{original_name}' -> '{protocol.name}'")
            logger.info(f"Questions count: {len(questions)}, Interventions count: {len(interventions)}, Decision nodes: {len(decision_tree)}")
            logger.info(f"Source: {content_data['source_document']}")
            logger.info(f"Content length: {len(content_data['content'])} characters")
            
            updated_count += 1
            
        except Exception as e:
            logger.error(f"Error updating protocol {protocol_type_str}: {e}")
            continue
    
    if updated_count > 0:
        try:
            db.session.commit()
            logger.info(f"✅ Successfully updated {updated_count} protocols")
            return True
        except Exception as e:
            db.session.rollback()
            logger.error(f"Error committing protocol updates: {e}")
            return False
    else:
        logger.warning("No protocols were updated")
        return False


def main():
    """Main function to run the protocol update."""
    logger.info("Protocol Update Script Starting...")
    ...
    with app.app_context():
        # Update protocols
        success = update_protocols_with_knowledge_content()
        
        if success:
            logger.info("🎉 Protocol update completed successfully!")
