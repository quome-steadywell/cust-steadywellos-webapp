    ...
    with app.app_context():
        ...
        for protocol_type, protocol_list in protocol_by_type.items():
            if len(protocol_list) > 1:
                ...
                print(f"Type {protocol_type}: Keeping ID {keep_protocol.id}, deleting IDs {[p.id for p in delete_protocols]}")
                ...
                for delete_protocol in delete_protocols:
                    ...
                    print(f"  Updating {len(assessments)} assessments from protocol {delete_protocol.id} to {keep_protocol.id}")
