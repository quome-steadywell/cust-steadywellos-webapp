    ...
    with app.app_context():
        ...
        if assessment_count < 5 and patient_count >= 3:
            ...
            if len(patients) >= 3 and len(nurses) >= 1 and len(protocols) >= 3:
                ...
                patient1 = next((p for p in patients if p.first_name == "John" and p.last_name == "Doe"), patients[0])
                patient2 = next((p for p in patients if p.first_name == "Mary" and p.last_name == "Johnson"), patients[1])
                patient3 = next((p for p in patients if p.first_name == "James" and p.last_name == "Wilson"), patients[2])
                
                # Find protocols by type
                cancer_protocol = next((p for p in protocols if p.protocol_type == ProtocolType.CANCER), protocols[0])
                heart_failure_protocol = next((p for p in protocols if p.protocol_type == ProtocolType.HEART_FAILURE), protocols[1])
                copd_protocol = next((p for p in protocols if p.protocol_type == ProtocolType.COPD), protocols[2])
