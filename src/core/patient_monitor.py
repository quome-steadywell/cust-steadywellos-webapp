    ...
    try:
        ...
        if patient:
            ...
            if not hasattr(patient, 'status'):
                ...
                logger.info(f"Patient model doesn't have status field, updating notes instead for {patient.first_name} {patient.last_name}")
            ...
            db.session.commit()
            logger.info(f"Successfully updated patient {patient.first_name} {patient.last_name} ({phone_number}) status to '{new_status}'")
            return True
        else:
...
def monitor_and_call() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Found {len(not_called_patients)} patients with 'not-called' status out of {len(patient_data)} total patients")
