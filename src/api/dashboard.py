...
def get_dashboard_summary():
    ...
    protocol_counts = (
        db.session.query(Patient.protocol_type, func.count(Patient.id))
        .filter(Patient.is_active == True)
        .group_by(Patient.protocol_type)
        .all()
    )

    protocol_distribution = {protocol_type.value: 0 for protocol_type in ProtocolType}
    ...
    pending_calls = call_query.filter(Call.status == CallStatus.SCHEDULED).count()
    ...
    assessments_this_week = assessment_query.filter(
        Assessment.assessment_date >= week_start, Assessment.assessment_date <= week_end
    ).count()
...
def get_recent_activity():
    ...
    call_query = Call.query.filter(Call.status == CallStatus.COMPLETED)
