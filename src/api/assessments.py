...
def get_all_assessments():
    ...
    if follow_up_priority:
        try:
            ...
            return (
                jsonify({"error": f"Invalid follow-up priority: {follow_up_priority}"}),
                400,
            )
...
def update_assessment(id):
    ...
    if (
        ("responses" in assessment_data or "protocol_id" in assessment_data)
        and assessment.protocol_id
        and not assessment.ai_guidance
    ):
        try:
            ai_guidance = process_assessment(assessment)
            assessment.ai_guidance = ai_guidance
...
def complete_followup(id):
    ...
    if not assessment.follow_up_needed:
        return (
            jsonify({"error": "This assessment doesn't have a follow-up scheduled"}),
            400,
        )
...
def get_followups():
    ...
    query = Assessment.query.filter(Assessment.follow_up_needed == True)
