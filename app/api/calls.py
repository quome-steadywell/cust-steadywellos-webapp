...
from app.schemas.call import CallSchema, CallListSchema, CallUpdateSchema, CallTranscriptSchema
from app.utils.decorators import roles_required, audit_action
from app.models.audit_log import AuditLog
from app.services.twilio_service import initiate_call, generate_call_twiml, process_call_recording
from app.services.rag_service import generate_call_script
    ...
    if patient_id:
        ...
        if current_user.role == UserRole.NURSE:
            ...
            if not patient or patient.primary_nurse_id != current_user_id:
                return jsonify({"error": "Unauthorized to view this patient's calls"}), 403
...
def cancel_call(id):
    ...
    if call.status != CallStatus.SCHEDULED:
        return jsonify({"error": f"Cannot cancel call with status {call.status.value}"}), 400
...
def initiate_outbound_call(id):
    ...
    if current_user.role == UserRole.NURSE:
        patient = Patient.query.get(call.patient_id)
        if not patient or patient.primary_nurse_id != current_user_id:
            return jsonify({"error": "Unauthorized to initiate call for this patient"}), 403
    
    # Can only initiate scheduled calls
    if call.status != CallStatus.SCHEDULED:
        return jsonify({"error": f"Cannot initiate call with status {call.status.value}"}), 400
    ...
    try:
        ...
        if call.call_type == 'assessment':
            ...
            if not protocol:
                return jsonify({"error": f"No active protocol found for {patient.protocol_type.value}"}), 400
...
def voice_webhook():
    ...
    if call:
        ...
        response.say("Hello, this is the palliative care coordination system. No active call was found for this number.")
        response.hangup()
...
def get_today_calls():
    ...
    query = Call.query.filter(
        Call.scheduled_time >= today_start,
        Call.scheduled_time <= today_end
    )
