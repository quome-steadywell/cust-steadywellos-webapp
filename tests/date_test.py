...
def test_dashboard_weekly_assessments(client):
    ...
    success, content = client.get(
        f"/api/v1/assessments/?from_date={from_date}&to_date={to_date}",
        expected_status=200,
    )
    if not success:
...
def test_weekly_assessment_boundary_cases(client):
    ...
    success, content = client.get(
        f"/api/v1/assessments/?from_date={last_week_start}&to_date={this_week_start}",
        expected_status=200,
    )
    if not success:
    ...
    success, content = client.get(
        f"/api/v1/assessments/?from_date={this_week_start}&to_date={next_week_start}",
        expected_status=200,
    )
    if not success:
    ...
    if len(this_week_assessments) == weekly_count:
        ...
        print(
            f"❌ Dashboard count ({weekly_count}) does not match API count for this week ({len(this_week_assessments)})"
        )
        return False
...
def test_sunday_inclusion(client):
    ...
    success, content = client.get(
        f"/api/v1/assessments/?from_date={sunday_start}&to_date={monday_start}",
        expected_status=200,
    )
    if not success:
    ...
    success, content = client.get(
        f"/api/v1/assessments/?from_date={monday_start}&to_date={next_week_start}",
        expected_status=200,
    )
    if not success:
